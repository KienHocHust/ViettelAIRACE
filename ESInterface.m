classdef ESInterface < handle
    ...
    methods
        ...
        function totalEpisodes = load_model(obj, path)
            totalEpisodes = 0; 
            if ~obj.isInitialized, return; end
            try
                % obj.pythonAgent.load_model(path);

                totalEpisodes = int32(obj.pythonAgent.load_model(path));

                fprintf('[MATLAB] Total Episode: %d\n', totalEpisodes);
            catch ME
                fprintf('[MATLAB] Error loading model: %s\n', ME.message);
            end
        end
